...
export class SkillDraftController extends FormApplication {
  ...
  _retrieveAllDraftableSkills() {
    ...
    try {
      ...
      console.error(
        "Paranoia | Could not dynamically load skills from data model. Falling back to defaults.",
        e,
      );
      ui.notifications.error(
        "Could not read skills from the Actor data model. Check console (F12) for details.",
      );
    }
  ...
  _handleSkillSelection(eventData) {
    ...
    if (actorId !== currentTurnActorId) {
      return console.warn(
        `Paranoia | Received skill selection from wrong player. Expected ${currentTurnActorId}, got ${actorId}.`,
      );
    }
  ...
  _handleStateRequest(eventData) {
    ...
    console.log(
      `Paranoia | Received state refresh request from user ${clientId}. Resending state.`,
    );
    game.socket.emit(socketEventChannel, {
      event: SkillDraftEvent.UPDATE_DRAFT_STATE,
      state: this.state,
      clientId: clientId,
    });
  }
  ...
  _startNextTurn() {
    ...
    this.state.currentPlayerName =
      this.state.playerNamesByActorId[this.state.participants[this.state.currentPlayerIndex]] ||
      "Unknown Player";
    this.state.nextPlayerName =
      this.state.playerNamesByActorId[this.state.participants[this.state.nextPlayerIndex]] ||
      "Unknown Player";
